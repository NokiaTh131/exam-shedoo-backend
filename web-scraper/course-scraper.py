...
# Alternative: Async version for even better performance
async def scrape_course_async(playwright, course_code: str):
    """Async version of course scraper"""
    try:
        browser = await playwright.chromium.launch(headless=True)
        page = await browser.new_page()
        await page.goto("https://www1.reg.cmu.ac.th/registrationoffice/searchcourse.php")
        
        await page.fill("#fcourse", course_code)
...
async def scrape_all_courses_async(start=0, end=999999, save_csv="all_courses.csv", max_concurrent=10):
    """
    ...
    async with async_playwright() as playwright:
        ...
        async def bounded_scrape(course_code):
            async with semaphore:
                return await scrape_course_async(playwright, course_code)
        
        print(f"Starting async scrape with {max_concurrent} concurrent requests...")
        
        with tqdm(total=len(course_codes), desc="Scraping courses") as pbar:
            tasks = [bounded_scrape(code) for code in course_codes]
